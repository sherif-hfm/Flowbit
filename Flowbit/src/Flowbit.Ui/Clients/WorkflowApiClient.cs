...
public sealed class WorkflowApiClient(HttpClient httpClient)
{
    ...
    public async Task<PagedResult<SharedVariableIncidentDto>> GetSharedVariableIncidentsAsync(
        string? status = "open",
        int page = 1,
        int pageSize = 50,
        CancellationToken cancellationToken = default)
    {
        var url = $"/api/shared-variable-incidents?page={Math.Max(1, page)}"
        ...
        using var response = await httpClient.GetAsync(url, cancellationToken);
        await EnsureSuccessAsync(response, cancellationToken);
        return await response.Content.ReadFromJsonAsync<PagedResult<SharedVariableIncidentDto>>(
                   cancellationToken)
            ?? throw new InvalidOperationException(
                "The API returned an empty shared-variable incident page.");
    }

    public async Task<SharedVariableIncidentDto?> GetSharedVariableIncidentAsync(
        long id,
        CancellationToken cancellationToken = default)
    {
        using var response = await httpClient.GetAsync(
            $"/api/shared-variable-incidents/{id}",
            cancellationToken);
        if (response.StatusCode == HttpStatusCode.NotFound)
        {
            return null;
        ...
        await EnsureSuccessAsync(response, cancellationToken);
        return await response.Content.ReadFromJsonAsync<SharedVariableIncidentDto>(
                   cancellationToken)
            ?? throw new InvalidOperationException(
                "The API returned an empty shared-variable incident.");
    }

    public async Task<SharedVariableIncidentDto> RetrySharedVariableIncidentAsync(
        long id,
        CancellationToken cancellationToken = default)
    {
        using var response = await httpClient.PostAsync(
            $"/api/shared-variable-incidents/{id}/retry",
            null,
            cancellationToken);
        await EnsureSuccessAsync(response, cancellationToken);
        return await response.Content.ReadFromJsonAsync<SharedVariableIncidentDto>(
                   cancellationToken)
            ?? throw new InvalidOperationException(
                "The API returned an empty retried shared-variable incident.");
    }

    public async Task<SharedVariableIncidentDto> ResolveSharedVariableIncidentAsync(
        long id,
        string reason,
        CancellationToken cancellationToken = default)
    {
        using var response = await httpClient.PostAsJsonAsync(
            $"/api/shared-variable-incidents/{id}/resolve",
            new ResolveSharedVariableIncidentRequest(reason),
            cancellationToken);
        await EnsureSuccessAsync(response, cancellationToken);
        return await response.Content.ReadFromJsonAsync<SharedVariableIncidentDto>(
                   cancellationToken)
            ?? throw new InvalidOperationException(
                "The API returned an empty resolved shared-variable incident.");
    }
