...
public sealed class SharedVariableRuntimeTests(PostgresApiFixture fixture)
{
    ...
    [Fact]
    public async Task ConditionalEntryReloadsExactSharedDependencyAfterConcurrentLockedUpdate()
    {
        var suffix = Guid.NewGuid().ToString("N");
        var sharedKey = $"tests.entry-race.{suffix}";
        await CreateSharedVariableAsync(
            sharedKey,
        ...
        var workflowId = await CreateWorkflowAsync(
            CreateWaiter($"shared-entry-race-{suffix}", sharedKey));

        await using var locker = fixture.CreateDbContext();
        await using var transaction = await locker.Database.BeginTransactionAsync();
        var variable = await locker.SharedVariables
            .FromSqlInterpolated(
                $"""SELECT * FROM flowbit.shared_variables WHERE "Key" = {sharedKey} FOR UPDATE""")
            .SingleAsync();
        var current = await locker.SharedVariableCurrentValues.SingleAsync(value =>
            value.SharedVariableId == variable.Id);

        // The start transaction first observes false without a lock, then must
        // block while acquiring this exact dependency row before conditional
        // entry. Updating behind the lock without emitting a wake makes a
        // stale, non-reloaded evaluation observable as a permanently lost wait.
        var startTask = StartAsync(workflowId);
        await WaitForBlockedSharedVariableLockAsync();
        current.ValueJson = JsonDocument.Parse("true");
        current.SetAt = DateTimeOffset.UtcNow;
        await locker.SaveChangesAsync();
        ...
        Assert.Equal(
            WorkflowInstanceStatuses.Completed,
            (await verify.WorkflowInstances.SingleAsync(item => item.Id == instance.Id)).Status);
    }
    ...
    [Fact]
    public async Task CrossWorkflowWriteWakesAtomicSharedConditionalWait()
    {
        var suffix = Guid.NewGuid().ToString("N");
        var sharedKey = $"tests.atomic-release.{suffix}";
        await CreateSharedVariableAsync(
            sharedKey,
            WorkflowVariableTypes.Boolean,
            nullable: false,
        ...
        var waiterWorkflowId = await CreateWorkflowAsync(
            CreateWaiter(
                $"shared-atomic-waiter-{suffix}",
                sharedKey,
                ConditionalEventDeliveryModes.Atomic));
        var writerWorkflowId = await CreateWorkflowAsync(
            CreateWriter($"shared-atomic-writer-{suffix}", sharedKey));
        var waiter = await StartAsync(waiterWorkflowId);
        var writer = await StartAsync(writerWorkflowId);
        ...
        await using (var before = fixture.CreateDbContext())
        {
            Assert.Equal(
                WorkflowInstanceStatuses.Running,
                (await before.WorkflowInstances.SingleAsync(item => item.Id == waiter.Id)).Status);
            writerTaskId = (await before.UserTasks.SingleAsync(task =>
                task.InstanceId == writer.Id
                && task.Status == UserTaskStatuses.Active)).Id;
        }
        ...
        using (var write = await SendAsync(
                   ...
                   {
                       ["release"] = JsonSerializer.SerializeToElement(true),
                       ["localNote"] = JsonSerializer.SerializeToElement("atomic wake")
                   }),
                   user: "writer"))
        {
            Assert.Equal(HttpStatusCode.OK, write.StatusCode);
        ...
        Assert.Equal(
            WorkflowInstanceStatuses.Completed,
            (await completed.WorkflowInstances.SingleAsync(item => item.Id == waiter.Id)).Status);
        Assert.False(await completed.WorkflowJobs.AnyAsync(job =>
            job.InstanceId == waiter.Id
            && job.Kind == WorkflowJobKinds.ConditionalWake));
        Assert.Equal(
            NodeExecutionCompletionReasons.ConditionalTriggered,
    ...
    private async Task ProcessSharedWakeOutboxAsync()
    {
        ...
        var deliveryRequest = request with
        {
            WorkerId = $"shared-delivery-test:{Guid.NewGuid():N}"
        };
    ...
    private static WorkflowModel CreateWaiter(
        string workflowKey,
        string sharedKey,
        string deliveryMode = ConditionalEventDeliveryModes.DurableAsync) => new()
    {
        Id = workflowKey,
        Name = workflowKey,
        InitialEventId = 1,
